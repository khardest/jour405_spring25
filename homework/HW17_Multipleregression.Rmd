...
In terms of correlation to the dependent variable, pct_unemployed seems to have a strong negative correlation with voter turnout, with a coefficient of -.74. pct_white has a strong positive correlation with turnout as there is a coefficient of .69.

In terms of independent variables, income and pct_bachelors_higher have a strong positive correlation with a coefficient of .75. population and pct_white have a strong negative correlation with a coefficient of -.68. 

The variable withthe lowest correlation to voter turnout is pct_bachelors_higher with a coefficient of .3. The lowest correlation overall is between pct_bachelors_higher and median_age with a coefficient of .039.

From the scatterplots, variables that have a clear linear relationship with voter turnout are pct_unemployed, pct_white, median_age and income. The ones with a weak linear relation are pct_bachelors_higher and population. 
...
The adjusted r squared value of .8023 shows that about 80% of the variation can be explained by the predictors. The p-value of 2.999e-06 is very small meaning the model can be considered statistically significant. The p values of income, median_age and pct_white were all below .05 meaning they were significant. population, pct_bachelors_higher and pct_unemployed were deemed insignificant based on their p-values. The median residual of .0005 shows that on average the model does a pretty accurate job in predicting. 

I would say this is an accurate model. Between the r squared value that can account for much of the variability and the multitude of variables I found to be statistically significant I would consider this model to be accurate and useful.
...
Model 2 performs about the same if not better than model 1 as the adjusted r squared value is .0001 higher. Since there are fewer variables involved I also feel as if model 2 is easier to interpret. Model 3 and 4 perform much worse with r squared values that are much lower. They explain much less variance in voter turnout. I would conclude that model 2 is the best choice. 
...
# Model 3
model3 <- lm(turnout ~ pct_white + pct_unemployed, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ income + median_age, data = md_turnout)
summary(model4)
...
I would use the data from this model to detail the factors that most seem to increase voter turnout in Maryland counties. Areas with higher income level, older populations and more white residence have had statistically higher voter turnout rates based on data. For a story I could go to counties with high voter turnout such as Caroll and Calvert county and interview community members to talk to them about voter turnout in their area. I could also talk to policy makers about how they plan to account for increased voter turnout among poorer and younger people, along with people of color. 
