...
Based on the regression line, the relationship looks like the higher the FARMS percentage, the higher the % of students who meet or exceed math scores will be. However, looking at the actual data points it dooesnt look like there is much of a relationship. 
...
I do not think this model is worth using. First of all, the p value is greater than .05 meaning the data is not statistically significant. Also, the adjusted r-squared value shows that only a very small amount of the math scores can be explained by the FARMS percentage. The large residuals also indicate that we cannot be confident that FARMS can explain all of the data. These two elements are most likely not related. 
...
The largest differences from the predicted and actual math scores are about 34 (White Marsh), 31 (Gulph), 31(Crooked Billet), 30 (Lower Gwynedd) and 29(Rydall). These large differences show that are model is not very accurate when it comes to predicting math scores. 
...
While there are some schools that indicate this relationship, the model and analysis indicate there is no real relationship between these two factors. From the analysis we concluded only 2.5% of the math scores can be explained by FARMS, and that the relationship was not significantly significant. Also, when using the model to predict math scores, we found very large differences in the predicted versus actual math scores for multiple elementary schools. 

This would mean the pitch for the story may explain the inaccuracy of the school board members statements through the data and information we found. We could explain that while some schools show this relationship, the relationship is not significant. 
